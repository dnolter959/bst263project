...
probabilities = matrix(0, K, n_models)
true_values = matrix(0, K, n_models)

for (j in 1:n) {
  # 1. extract indices of units in the pseudo-test set for split j
  pseudotest = data_complete[j, ]  
  # 2. extract indices of units in the pseudo-training set for split j
  pseudotrain = data_complete[-j, ]
  # 3. fit all models of interest
  mod1 = glm(aware ~ .-SEQN, data = pseudotrain, family = binomial)
  mod2 = glm(aware ~ smoker + education + married + age + race + income_poverty_ratio, data = pseudotrain, family = binomial())
  mod3 = glm(aware ~ education + married + race + number_ppl_household + age_menarche + period_past_yr + parity + past_yr_12drinks, data = pseudotrain, family = binomial())
  ...
  # 4. Compute Performance on each model
  for (i in 1:n_models) {
    probability = eval(parse(text = paste0("mod", i))) %>% predict(pseudotest, type = "response")
...
## Functions for Tables/Plots
```{r}
# Create table using true_values and probabilities matrices
create_table_for_cutoff = function(probabilities, true_values, cutoff){
  # Calculate Test Performance for cutoff value
...
create_roc_curve_plot = function(probabilities, true_values){
  roc_mod1 <- roc(true_values[, 1] - 1, probabilities[, 1])
  roc_mod2 <- roc(true_values[, 2] - 1, probabilities[, 2])
  roc_mod3 <- roc(true_values[, 3] - 1, probabilities[, 3])
  roc_mod4 <- roc(true_values[, 4] - 1, probabilities[, 4])
  
  ggroc(list("Losistic - All" = roc_mod1, "Losistic - Some" = roc_mod2, "Losistic - LASSO-selected" = roc_mod3, "GAM - LASSO-selected" = roc_mod4)) +
...
```{r}
# Example use
create_table_for_cutoff(probabilities, true_values, 0.5)
create_roc_curve_plot(probabilities, true_values)
```